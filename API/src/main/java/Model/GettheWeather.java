...
public class GettheWeather {
	...
	//method that gets wanted value from API response
	private static String getValOfXML(String ApiResponse, String searchFor, String valName){
		...
		NodeList nList = doc.getElementsByTagName(searchFor);
		...
		for (int temp = 0; temp < nList.getLength(); temp++) {
			...
			if (node.getNodeType() == Node.ELEMENT_NODE) {
				...
				returnVal = eElement.getAttribute(valName);
	...
	public static void getWeather(weatherBean wBean) throws IOException {
		String API_KEY = "100c23d8b30413be983af196a6d584c7";
		...
		wBean.setCloudsStr(getValOfXML(ApiResponse, "clouds", "name"));
		wBean.setLastUpdate(getValOfXML(ApiResponse, "lastupdate", "value"));
		wBean.setTemperature(getValOfXML(ApiResponse, "temperature", "value"));
		wBean.setWind(getValOfXML(ApiResponse, "speed", "name"));
